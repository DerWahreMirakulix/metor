...
from .descriptors import SAFE_SETTING_LABELS, setting_descriptors
...
class ConfigCommandHandler:
    ...
    def handle(self, cmd: IpcCommand) -> IpcEvent:
        ...
        if isinstance(cmd, SetConfigCommand):
            ...
            try:
                if cmd.safe_only and set_config_key not in SAFE_SETTING_LABELS:
        ...
        if isinstance(cmd, GetConfigListCommand):
            if cmd.safe_descriptors:
                return ConfigListDataEvent(
                    'daemon', self._pm.profile_name, setting_descriptors(self._pm)
                )
            return create_event(
