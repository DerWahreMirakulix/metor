"""Thread-safe network state coordinator and emission ownership."""
...
class StateTracker(
    ...
    def set_peer_writer_failure_callback(
        self, callback: Callable[[str, socket.socket], None]
    ) -> None:
    ...
    def finish_connection(self, conn: socket.socket, frame: bytes) -> None:
        """Transfers final-frame delivery and bounded closure to the sole writer.

        Args:
            conn (socket.socket): Exact retiring socket.
            frame (bytes): Final peer control message.
