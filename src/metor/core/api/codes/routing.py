"""Routing enums shared across the IPC boundary."""

from enum import Enum


class CommandType(str, Enum):
    """Enumeration of commands sent from the UI or CLI to the daemon."""

    REGISTER_VOICE_OWNER = 'register_voice_owner'
    RELEASE_VOICE_OWNER = 'release_voice_owner'
    GET_MESSAGE_OUTCOME = 'get_message_outcome'
    GET_GUI_PREFERENCES = 'get_gui_preferences'
    SET_GUI_PREFERENCES = 'set_gui_preferences'
    INIT = 'init'
    GET_CHAT_STARTUP_STATE = 'get_chat_startup_state'
    GET_CONNECTIONS = 'get_connections'
    GET_TRANSPORT_STATE = 'get_transport_state'
    GET_CONTACTS_LIST = 'get_contacts_list'
    CONNECT = 'connect'
    DISCONNECT = 'disconnect'
    ACCEPT = 'accept'
    REJECT = 'reject'
    SEND_MESSAGE = 'send_message'
    ADD_CONTACT = 'add_contact'
    REMOVE_CONTACT = 'remove_contact'
    RENAME_CONTACT = 'rename_contact'
    CLEAR_CONTACTS = 'clear_contacts'
    SWITCH = 'switch'
    GET_INBOX = 'get_inbox'
    MARK_READ = 'mark_read'
    FALLBACK = 'fallback'
    DELETE_MESSAGE = 'delete_message'
    DISMISS_LIVE_CONTEXT = 'dismiss_live_context'
    GET_RUNTIME_SNAPSHOT = 'get_runtime_snapshot'
    GET_HISTORY = 'get_history'
    GET_RAW_HISTORY = 'get_raw_history'
    CLEAR_HISTORY = 'clear_history'
    GET_MESSAGES = 'get_messages'
    CLEAR_MESSAGES = 'clear_messages'
    GET_ADDRESS = 'get_address'
    GENERATE_ADDRESS = 'generate_address'
    CLEAR_PROFILE_DB = 'clear_profile_db'
    ADD_PROFILE = 'add_profile'
    MIGRATE_PROFILE_SECURITY = 'migrate_profile_security'
    REMOVE_PROFILE = 'remove_profile'
    RENAME_PROFILE = 'rename_profile'
    SET_DEFAULT_PROFILE = 'set_default_profile'
    SET_SETTING = 'set_setting'
    GET_SETTING = 'get_setting'
    GET_SETTINGS_LIST = 'get_settings_list'
    SET_CONFIG = 'set_config'
    GET_CONFIG = 'get_config'
    GET_CONFIG_LIST = 'get_config_list'
    SYNC_CONFIG = 'sync_config'
    SELF_DESTRUCT = 'self_destruct'
    UNLOCK = 'unlock'
    LOCK = 'lock'
    CHANGE_PASSWORD = 'change_password'
    AUTHENTICATE_SESSION = 'authenticate_session'
    RETUNNEL = 'retunnel'
    REGISTER_LIVE_CONSUMER = 'register_live_consumer'
    RESTRICT_CLIENT = 'restrict_client'
    GET_RESTRICTED_CLIENT_STATE = 'get_restricted_client_state'
    REAUTHORIZE_CLIENT = 'reauthorize_client'
    CONFIGURE_QUICK_UNLOCK = 'configure_quick_unlock'
    BEGIN_VOICE = 'begin_voice'
    APPEND_VOICE_CHUNK = 'append_voice_chunk'
    FINALIZE_VOICE = 'finalize_voice'
    GET_VOICE_CHUNK = 'get_voice_chunk'
    LIST_RETAINED_MESSAGES = 'list_retained_messages'
    RELEASE_VOICE = 'release_voice'
    COMMIT_VOICE = 'commit_voice'
    CANCEL_VOICE = 'cancel_voice'
    PREPARE_PROFILE_EXIT = 'prepare_profile_exit'


class EventType(str, Enum):
    """Enumeration of strict daemon-to-UI events."""

    INIT = 'init'
    CHAT_STARTUP_STATE = 'chat_startup_state'
    TOR_KEY_DECRYPT_FAILED = 'tor_key_decrypt_failed'
    TOR_KEY_WRITE_FAILED = 'tor_key_write_failed'
    TOR_START_FAILED = 'tor_start_failed'
    TOR_PROCESS_TERMINATED = 'tor_process_terminated'
    MESSAGE_RECEIVED = 'message_received'
    ACK = 'ack'
    DROP_FAILED = 'drop_failed'
    CONNECTED = 'connected'
    DISCONNECTED = 'disconnected'
    CONNECTION_CONNECTING = 'connection_connecting'
    RENAME_SUCCESS = 'rename_success'
    CONTACT_REMOVED = 'contact_removed'
    CONNECTIONS_STATE = 'connections_state'
    SWITCH_SUCCESS = 'switch_success'
    CONNECTION_PENDING = 'connection_pending'
    CONNECTION_AUTO_ACCEPTED = 'connection_auto_accepted'
    CONNECTION_RETRY = 'connection_retry'
    CONNECTION_FAILED = 'connection_failed'
    INCOMING_CONNECTION = 'incoming_connection'
    CONNECTION_REJECTED = 'connection_rejected'
    AUTO_RECONNECT_SCHEDULED = 'auto_reconnect_scheduled'
    INBOX_NOTIFICATION = 'inbox_notification'
    INBOX_DATA = 'inbox_data'
    CONTACTS_DATA = 'contacts_data'
    HISTORY_DATA = 'history_data'
    HISTORY_RAW_DATA = 'history_raw_data'
    MESSAGES_DATA = 'messages_data'
    INBOX_COUNTS = 'inbox_counts'
    UNREAD_MESSAGES = 'unread_messages'
    ADDRESS_CURRENT = 'address_current'
    ADDRESS_GENERATED = 'address_generated'
    ADDRESS_CANT_GENERATE_RUNNING = 'address_cant_generate_running'
    ADDRESS_NOT_GENERATED = 'address_not_generated'
    PROFILES_DATA = 'profiles_data'
    AUTH_REQUIRED = 'auth_required'
    INVALID_PASSWORD = 'invalid_password'
    LOCAL_AUTH_RATE_LIMITED = 'local_auth_rate_limited'
    DB_CORRUPTED = 'db_corrupted'
    ALREADY_UNLOCKED = 'already_unlocked'
    SESSION_AUTHENTICATED = 'session_authenticated'
    SELF_DESTRUCT_INITIATED = 'self_destruct_initiated'
    PASSWORD_CHANGED = 'password_changed'
    PASSWORD_CHANGE_UNSUPPORTED = 'password_change_unsupported'
    INVALID_NEW_PASSWORD = 'invalid_new_password'
    PASSWORD_CHANGE_FAILED = 'password_change_failed'
    DAEMON_UNLOCKED = 'daemon_unlocked'
    DAEMON_LOCKED = 'daemon_locked'
    DAEMON_OFFLINE = 'daemon_offline'
    IPC_CLIENT_LIMIT_REACHED = 'ipc_client_limit_reached'
    PROTOCOL_MISMATCH = 'protocol_mismatch'
    UNKNOWN_COMMAND = 'unknown_command'
    INTERNAL_ERROR = 'internal_error'
    INVALID_SETTING_KEY = 'invalid_setting_key'
    INVALID_CONFIG_KEY = 'invalid_config_key'
    CLIENT_SCOPE_KEY_REJECTED = 'client_scope_key_rejected'
    SETTING_UPDATED = 'setting_updated'
    SETTING_UPDATE_FAILED = 'setting_update_failed'
    SETTING_TYPE_ERROR = 'setting_type_error'
    SETTING_DATA = 'setting_data'
    SETTINGS_LIST_DATA = 'settings_list_data'
    CONFIG_UPDATED = 'config_updated'
    CONFIG_UPDATE_FAILED = 'config_update_failed'
    CONFIG_DATA = 'config_data'
    CONFIG_LIST_DATA = 'config_list_data'
    CONFIG_SYNCED = 'config_synced'
    CANNOT_CONNECT_SELF = 'cannot_connect_self'
    INVALID_TARGET = 'invalid_target'
    CANNOT_SWITCH_SELF = 'cannot_switch_self'
    NO_CONNECTION_TO_REJECT = 'no_connection_to_reject'
    NO_CONNECTION_TO_DISCONNECT = 'no_connection_to_disconnect'
    NO_PENDING_CONNECTION = 'no_pending_connection'
    PENDING_CONNECTION_EXPIRED = 'pending_connection_expired'
    MAX_CONNECTIONS_REACHED = 'max_connections_reached'
    DROPS_DISABLED = 'drops_disabled'
    CANNOT_DROP_SELF = 'cannot_drop_self'
    DROP_QUEUED = 'drop_queued'
    AUTO_FALLBACK_QUEUED = 'auto_fallback_queued'
    LIVE_MESSAGE_UNAVAILABLE = 'live_message_unavailable'
    LIVE_MESSAGE_RESOURCE_PRESSURE = 'live_message_resource_pressure'
    NO_PENDING_LIVE_MSGS = 'no_pending_live_msgs'
    FALLBACK_SUCCESS = 'fallback_success'
    FALLBACK_REJECTED = 'fallback_rejected'
    CONTACT_ADDED = 'contact_added'
    PEER_NOT_FOUND = 'peer_not_found'
    DISCOVERED_PEER_NOT_FOUND = 'discovered_peer_not_found'
    CONTACT_ALREADY_SAVED = 'contact_already_saved'
    PEER_PROMOTED = 'peer_promoted'
    ALIAS_IN_USE = 'alias_in_use'
    ONION_IN_USE = 'onion_in_use'
    ALIAS_SAME = 'alias_same'
    ALIAS_NOT_FOUND = 'alias_not_found'
    ALIAS_RENAMED = 'alias_renamed'
    PEER_CANT_DELETE_ACTIVE = 'peer_cant_delete_active'
    CONTACT_DOWNGRADED = 'contact_downgraded'
    CONTACT_REMOVED_DOWNGRADED = 'contact_removed_downgraded'
    PEER_ANONYMIZED = 'peer_anonymized'
    PEER_REMOVED = 'peer_removed'
    CONTACTS_CLEARED = 'contacts_cleared'
    CONTACTS_CLEAR_FAILED = 'contacts_clear_failed'
    HISTORY_CLEARED = 'history_cleared'
    HISTORY_CLEARED_ALL = 'history_cleared_all'
    HISTORY_CLEAR_FAILED = 'history_clear_failed'
    MESSAGES_CLEARED = 'messages_cleared'
    MESSAGES_CLEARED_NON_CONTACTS = 'messages_cleared_non_contacts'
    MESSAGES_CLEARED_NON_CONTACTS_ALL = 'messages_cleared_non_contacts_all'
    MESSAGES_CLEARED_ALL = 'messages_cleared_all'
    MESSAGES_CLEAR_FAILED = 'messages_clear_failed'
    DB_CLEARED = 'db_cleared'
    DB_CLEAR_FAILED = 'db_clear_failed'
    PROFILE_OPERATION_RESULT = 'profile_operation_result'
    RETUNNEL_INITIATED = 'retunnel_initiated'
    RETUNNEL_SUCCESS = 'retunnel_success'
    RETUNNEL_FAILED = 'retunnel_failed'
    TRANSPORT_STATE = 'transport_state'
    READ_RECEIPT = 'read_receipt'
    MESSAGE_DELETED = 'message_deleted'
    MESSAGE_DELETE_REJECTED = 'message_delete_rejected'
    LIVE_CONTEXT_DISMISSED = 'live_context_dismissed'
    LIVE_CONTEXT_DISMISS_REJECTED = 'live_context_dismiss_rejected'
    LIVE_CONTROL_REJECTED = 'live_control_rejected'
    LIVE_CONTROL_COMPLETED = 'live_control_completed'
    RUNTIME_SNAPSHOT = 'runtime_snapshot'
    RUNTIME_SNAPSHOT_UNAVAILABLE = 'runtime_snapshot_unavailable'
    RUNTIME_STATE_CHANGED = 'runtime_state_changed'
    VOICE_STARTED = 'voice_started'
    VOICE_CHUNK_ACCEPTED = 'voice_chunk_accepted'
    VOICE_CHUNK_RECEIVED = 'voice_chunk_received'
    VOICE_FINALIZED = 'voice_finalized'
    VOICE_RESOURCE_PRESSURE = 'voice_resource_pressure'
    VOICE_RESOURCE_LIMIT = 'voice_resource_limit'
    VOICE_DATA = 'voice_data'
    RETAINED_MESSAGES = 'retained_messages'
    RETAINED_MESSAGES_UNAVAILABLE = 'retained_messages_unavailable'
    VOICE_RELEASED = 'voice_released'
    VOICE_OPERATION_REJECTED = 'voice_operation_rejected'
    VOICE_INCOMING_STARTED = 'voice_incoming_started'
    VOICE_COMMITTED = 'voice_committed'
    VOICE_CANCELLED = 'voice_cancelled'
    CLIENT_RESTRICTED = 'client_restricted'
    RESTRICTED_CLIENT_STATE = 'restricted_client_state'
    CLIENT_REAUTHORIZED = 'client_reauthorized'
    CLIENT_ACCESS_RESTRICTED = 'client_access_restricted'
    QUICK_UNLOCK_CONFIGURED = 'quick_unlock_configured'
    QUICK_UNLOCK_FAILED = 'quick_unlock_failed'
    PROFILE_EXIT_PREPARED = 'profile_exit_prepared'
    SELF_DESTRUCT_COMPLETED = 'self_destruct_completed'
    SELF_DESTRUCT_KEY_DESTROYED = 'self_destruct_key_destroyed'
    SELF_DESTRUCT_RUNTIME_RELEASED = 'self_destruct_runtime_released'
    SELF_DESTRUCT_SAFE = 'self_destruct_safe'
    SELF_DESTRUCT_CLEANUP_FAILED = 'self_destruct_cleanup_failed'
    VOICE_OWNER_REGISTERED = 'voice_owner_registered'
    VOICE_OWNER_RELEASED = 'voice_owner_released'
    VOICE_OWNER_REJECTED = 'voice_owner_rejected'
    MESSAGE_OUTCOME = 'message_outcome'
    TEXT_ACCEPTED = 'text_accepted'
    TEXT_REJECTED = 'text_rejected'
    GUI_PREFERENCES = 'gui_preferences'
    GUI_PREFERENCES_REJECTED = 'gui_preferences_rejected'
