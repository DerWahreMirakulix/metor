...
from metor.cli.dispatcher.messages import MessagesDispatchMixin
from metor.cli.dispatcher.profiles import ProfilesDispatchMixin
...
class CliDispatcher(ProfilesDispatchMixin, MessagesDispatchMixin, HistoryDispatchMixin):
    ...
    def dispatch(self) -> int:
        ...
        if cmd == 'quickstart':
            ...
            if getattr(self._args, 'chat_help', False):
                print(self._help.show_chat_launcher_help())
            elif self._extra:
                self._print_usage(cmd)
            else:
                loaded_frontend = getattr(self._args, 'loaded_frontend', None)
