...
def _format_snapshot_source(source: str) -> str:
    ...
    return labels.get(source, escape_terminal_text(source.replace('_', ' ')))
...
def _format_snapshot_category(category: str) -> str:
    ...
    if category.startswith('ui.'):
        ...
        return escape_terminal_text(frontend_id.replace('_', ' ').title())

    return escape_terminal_text(category)
...
def _format_snapshot_entries(
    ...
    *,
    ...
    for entry in entries:
        ...
        lines.append(
            '  '
            f'{Theme.CYAN}{escape_terminal_text(entry.key)}{Theme.RESET} = '
            f'{Theme.YELLOW}{escape_terminal_text(str(entry.value))}{Theme.RESET} '
...
def format_settings_snapshot(event: SettingsListDataEvent) -> str:
    ...
    if event.scope == 'client':
        ...
        header = f'Global {escape_terminal_text(event.scope)} Settings'
    return '\n'.join(
        [
            header + ':',
            '',
...
def format_config_snapshot(event: ConfigListDataEvent) -> str:
    ...
    if event.scope == 'client':
        header = (
            'Effective Client Config for profile '
            f"'{escape_terminal_text(event.profile)}'"
        )
    elif event.scope == 'daemon':
        header = (
            'Effective Daemon Config for profile '
            f"'{escape_terminal_text(event.profile)}'"
        )
    else:
        header = (
            'Structural Profile Config for profile '
            f"'{escape_terminal_text(event.profile)}'"
        )
...
def format_contacts(event: ContactsDataEvent, chat_mode: bool) -> str:
    ...
    """
    profile_suffix: str = (
        '' if chat_mode else f" for profile '{escape_terminal_text(event.profile)}'"
    )
    lines: List[str] = []
    ...
    if event.saved:
        ...
        for entry in event.saved:
            lines.append(
                f'   {Theme.GREEN}{escape_terminal_text(entry.alias)}{Theme.RESET}'
                f' -> {escape_terminal_text(entry.onion)}'
            )
    else:
    ...
    if event.discovered:
        ...
        for entry in event.discovered:
            lines.append(
                f'   {Theme.DARK_GREY}{escape_terminal_text(entry.alias)}'
                f'{Theme.RESET} -> {escape_terminal_text(entry.onion)}'
            )
...
def format_messages(event: MessagesDataEvent) -> str:
    ...
    if not event.messages:
        return f"No chat history found for '{escape_terminal_text(event.alias)}'."

    safe_alias: str = escape_terminal_text(event.alias)

    header_text: str = (
        f'Chat History with {Theme.CYAN}{safe_alias}{Theme.RESET} '
        f'(Last {len(event.messages)})'
    )
    out: str = f'{get_header_string(header_text)}'
    for msg in event.messages:
        if msg.direction is MessageDirectionCode.OUT:
            prefix_text: str = f'To {safe_alias}: '
...
def format_inbox(event: InboxCountsEvent) -> str:
    ...
    for alias, count in event.inbox.items():
        out += (
            f' - {Theme.PURPLE}{escape_terminal_text(alias)}{Theme.RESET}: '
...
def format_read_messages(event: UnreadMessagesEvent) -> str:
    ...
    """
    if not event.messages:
        return f"No unread messages from '{escape_terminal_text(event.alias)}'."

    safe_alias: str = escape_terminal_text(event.alias)

    header_text: str = f'Unread messages from {Theme.PURPLE}{safe_alias}{Theme.RESET}'
    out: str = f'{get_header_string(header_text)}'
    for msg in event.messages:
        prefix_text: str = f'From {safe_alias}: '
        rendered_prefix_text: str = f'{Theme.PURPLE}{prefix_text}{Theme.RESET}'
        timestamp_prefix, timestamp_visible = build_timestamp_prefix(
            msg.timestamp,
            is_drop=msg.delivery is Delivery.DROP,
        )
        out += (
            format_prefixed_message(
                f'{timestamp_prefix}{rendered_prefix_text}',
...
def format_profiles(event: ProfilesDataEvent) -> str:
    ...
    for profile in event.profiles:
        ...
        if profile.is_active:
            lines.append(
                f' {Theme.GREEN}{marker} {escape_terminal_text(profile.name)}'
                f'{Theme.RESET}{tag_str}'
            )
        else:
            lines.append(f'   {escape_terminal_text(profile.name)}{tag_str}')
