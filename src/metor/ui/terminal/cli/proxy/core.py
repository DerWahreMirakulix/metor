...
from metor.ui.terminal.cli.proxy.profiles import CliProxyProfileActions
from metor.ui.terminal.cli.proxy.rendering import CliProxyEventRenderer
...
class CliProxy:
    ...
    def lock_daemon(self) -> str:
        """Locks the profile runtime without shutting down the daemon.

        Args:
            None

        Returns:
            str: Rendered daemon response.
        """
        err: Optional[str] = self._ensure_profile_exists()
        if err:
            return err
        return self._request_ipc(LockCommand(), wait_for_response=True)
