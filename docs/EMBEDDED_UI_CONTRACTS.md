...
| DROP detail/send | `MessagePage`, typed delivery states | `SendMessageCommand(DROP, content)` | Core | text supported |
...
| LIVE text stream | transient `MessageReceivedEvent(LIVE, TextContent)` | `SendMessageCommand(LIVE, TextContent)` | Core | supported |
